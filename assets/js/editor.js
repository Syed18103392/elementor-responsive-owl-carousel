(function ($) {
	elementor.hooks.addAction(
		...
		function (panel, model, view) {
			...
			gf_owl_carousel_repeater_field_hide($panelEl);
			...
			$panelEl.on(
				...
				function (e) {
					gf_owl_carousel_do_bunch_of_stuff($(this), $panelEl, $viewEl, e);
				}
			);
	...
	function gf_owl_carousel_do_bunch_of_stuff($this, $panelEl, $viewEl, event) {
		...
		if ($this.is("select")) {
			if (dataSetting === "carousel_layout") {
				...
				gf_owl_carousel_modify_setting($panelEl, val);
			}
		}

		if ($this.closest(".js_repeater_single").length) {
			gf_owl_carousel_update_carousel_item($this, $viewEl, dataSetting, val, event);
		}
	}

	// hide repeater fields depending on layout
	function gf_owl_carousel_repeater_field_hide($element) {
		var $repeater = $element.find(".js_items_list_repeater");
		var currentLayout = gf_owl_carousel_find_by_data(
			$element,
			"carousel_layout"
		).val();
	...
	} // gf_owl_carousel_repeater_field_hide

	function gf_owl_carousel_update_carousel_item( $this, $element, currentSetting, currentVal, event ) {
		...
		if (currentSetting !== "url") {
			gf_owl_carousel_find_by_data($currentViewItem, currentSetting).html(
				currentVal
			);
		}

		// fires when focused out of the input/textarea
		if (event.type === "change" && currentSetting === "url") {
			$this.trigger("input");
		}
	} // gf_owl_carousel_update_carousel_item

	function gf_owl_carousel_modify_setting( $element, currentVal ) {
		var imgSize = gf_owl_carousel_find_by_data($element, "carousel_thumbnail_size");
	...
	} // gf_owl_carousel_modify_setting

	// helpers
	function gf_owl_carousel_find_by_data(element, val, attr = "setting") {
